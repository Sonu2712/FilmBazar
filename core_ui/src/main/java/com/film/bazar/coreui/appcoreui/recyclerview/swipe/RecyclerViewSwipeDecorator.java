package com.film.bazar.coreui.appcoreui.recyclerview.swipe;
...
/**
 * A simple utility class to add a background and/or an icon while swiping a RecyclerView item left
 * or right.
 */
...
public class RecyclerViewSwipeDecorator {
    ...
    private float mSwipeLeftTextSize = 18;
    ...
    private float mSwipeRightTextSize = 18;
     ...
     * @param actionState The type of interaction on the View. Is either ACTION_STATE_DRAG or
     * ACTION_STATE_SWIPE.
     * @param isCurrentlyActive True if this view is currently being controlled by the user or false
     * it is simply animating back to its original state
     * @deprecated in RecyclerViewSwipeDecorator 1.2.2
     */
    @Deprecated public RecyclerViewSwipeDecorator(Context context, Canvas canvas,
     ...
     * it is simply animating back to its original state
     */
    public RecyclerViewSwipeDecorator(Canvas canvas, RecyclerView recyclerView,
        ...
        this.iconHorizontalMargin = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 16,
     ...
     */
    @Deprecated public void setIconHorizontalMargin(int iconHorizontalMargin) {
    ...
    public void setIconHorizontalMargin(int unit, int iconHorizontalMargin) {
        this.iconHorizontalMargin = (int) TypedValue.applyDimension(unit, iconHorizontalMargin,
            recyclerView.getContext().getResources().getDisplayMetrics());
    }
            ...
            if (dX > 0) {
                ...
                if (swipeRightBackgroundColor != 0) {
                    ...
                    background.setBounds(viewHolder.itemView.getLeft(),
                        viewHolder.itemView.getTop(), viewHolder.itemView.getLeft() + (int) dX,
                ...
                if (mSwipeRightText != null
                    ...
                    && dX > iconHorizontalMargin + iconSize) {
                    TextPaint textPaint = new TextPaint();
                    textPaint.setAntiAlias(true);
                    textPaint.setTextSize(
                        TypedValue.applyDimension(mSwipeRightTextUnit, mSwipeRightTextSize,
                            recyclerView.getContext().getResources().getDisplayMetrics()));
                    textPaint.setColor(mSwipeRightTextColor);
                    textPaint.setTypeface(mSwipeRightTypeface);

                    int textTop =
                        (int) (viewHolder.itemView.getTop() + ((viewHolder.itemView.getBottom()
                            - viewHolder.itemView.getTop()) / 2.0) + textPaint.getTextSize() / 2);
                    canvas.drawText(mSwipeRightText,
                        viewHolder.itemView.getLeft() + iconHorizontalMargin + iconSize + (
                            iconSize > 0 ? iconHorizontalMargin / 2 : 0), textTop, textPaint);
                }
            } else if (dX < 0) {
                // Swiping Left
                ...
                if (swipeLeftActionIconId != 0 && dX < -iconHorizontalMargin) {
                    Drawable icon =
                        ContextCompat.getDrawable(recyclerView.getContext(), swipeLeftActionIconId);
                    if (icon != null) {
                        iconSize = icon.getIntrinsicHeight();
                        int halfIcon = iconSize / 2;
                        int top = viewHolder.itemView.getTop() + ((viewHolder.itemView.getBottom()
                            - viewHolder.itemView.getTop()) / 2 - halfIcon);
                        imgLeft =
                            viewHolder.itemView.getRight() - iconHorizontalMargin - halfIcon * 2;
                ...
                if (mSwipeLeftText != null
                    ...
                    textPaint.setAntiAlias(true);
                    textPaint.setTextSize(
                        TypedValue.applyDimension(mSwipeLeftTextUnit, mSwipeLeftTextSize,
                            recyclerView.getContext().getResources().getDisplayMetrics()));
                    ...
                    canvas.drawText(mSwipeLeftText,
                        imgLeft - width - (imgLeft == viewHolder.itemView.getRight()
                            ? iconHorizontalMargin : iconHorizontalMargin / 2), textTop, textPaint);
                }
            }
        } catch (Exception e) {
    ...
    public static class Builder {
         ...
         * @deprecated in RecyclerViewSwipeDecorator 1.2.2
         */
        @Deprecated public Builder(Context context, Canvas canvas, RecyclerView recyclerView,
            RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState,
            boolean isCurrentlyActive) {
            this(canvas, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive);
        }
         ...
         * false it is simply animating back to its original state
         */
        public Builder(Canvas canvas, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder,
            float dX, float dY, int actionState, boolean isCurrentlyActive) {
            mDecorator = new RecyclerViewSwipeDecorator(canvas, recyclerView, viewHolder, dX, dY,
         ...
         * @return This instance of @RecyclerViewSwipeDecorator.Builder
         * @deprecated in RecyclerViewSwipeDecorator 1.2, use {@link #setIconHorizontalMargin(int,
         * int)} instead.
         */
        @Deprecated public Builder setIconHorizontalMargin(int pixels) {
