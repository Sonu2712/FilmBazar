...
public class SwipeLayout extends FrameLayout {
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            int evLeft = surfaceView.getLeft(), evRight = surfaceView.getRight(), evTop =
                surfaceView.getTop(), evBottom = surfaceView.getBottom();
                ...
                switch (mCurrentDragEdge) {
                    ...
                    case Top:
                        if (mShowMode == ShowMode.PullOut) {
                            ...
                            if (surfaceViewTop + dy < getPaddingTop()) return getPaddingTop();
    ...
    /**
     * save children's bounds, so they can restore the bound in {@link #onLayout(boolean, int, int,
     * int, int)}
     */
    ...
    protected boolean isViewTotallyFirstShowed(View child, Rect relativePosition, DragEdge edge,
        int surfaceLeft, int surfaceTop, int surfaceRight, int surfaceBottom) {
        ...
        if (getShowMode() == ShowMode.LayDown) {
            if ((edge == DragEdge.Right && surfaceRight <= childLeft)
                || (edge == DragEdge.Left
                && surfaceLeft >= childRight)
                || (edge == DragEdge.Top && surfaceTop >= childBottom)
                || (edge == DragEdge.Bottom && surfaceBottom <= childTop)) {
                r = true;
            }
        } else if (getShowMode() == ShowMode.PullOut) {
            if ((edge == DragEdge.Right && childRight <= getWidth()) || (edge == DragEdge.Left
                && childLeft >= getPaddingLeft()) || (edge == DragEdge.Top
                && childTop >= getPaddingTop()) || (edge == DragEdge.Bottom
                && childBottom <= getHeight())) {
                r = true;
            }
        }
        return r;
    ...
    protected boolean isViewShowing(View child, Rect relativePosition, DragEdge availableEdge,
        int surfaceLeft, int surfaceTop, int surfaceRight, int surfaceBottom) {
        int childLeft = relativePosition.left;
        int childRight = relativePosition.right;
        int childTop = relativePosition.top;
    ...
    protected void dispatchRevealEvent(final int surfaceLeft, final int surfaceTop,
        ...
        for (Map.Entry<View, ArrayList<OnRevealListener>> entry : mRevealListeners.entrySet()) {
            ...
            if (isViewShowing(child, rect, mCurrentDragEdge, surfaceLeft, surfaceTop, surfaceRight,
                surfaceBottom)) {
                mShowEntirely.put(child, false);
                int distance = 0;
                float fraction = 0f;
    ...
    private void checkCanDrag(MotionEvent ev) {
        ...
        if (mCurrentDragEdge == DragEdge.Right) {
            boolean suitable =
                (status == Status.Open && distanceX > mTouchSlop) || (status == Status.Close
        ...
        if (mCurrentDragEdge == DragEdge.Top) {
            boolean suitable =
                (status == Status.Open && distanceY < -mTouchSlop) || (status == Status.Close
    ...
    private void performAdapterViewItemClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            if (p != AdapterView.INVALID_POSITION) {
                view.performItemClick(view.getChildAt(p - view.getFirstVisiblePosition()), p,
    ...
    private boolean performAdapterViewItemLongClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            try {
                Method m =
                    AbsListView.class.getDeclaredMethod("performLongPress", View.class, int.class,
                        long.class);
     ...
     */
    @Deprecated public void setDragEdge(DragEdge dragEdge) {
        clearDragEdge();
        if (getChildCount() >= 2) {
     ...
     */
    @Deprecated public List<DragEdge> getDragEdges() {
     ...
     * Deprecated, use {@link #setDrag(DragEdge, View)}
     */
    @Deprecated public void setDragEdges(List<DragEdge> dragEdges) {
        clearDragEdge();
     ...
     */
    @Deprecated public void setDragEdges(DragEdge... mDragEdges) {
        clearDragEdge();
     ...
     */
    @Deprecated public void setBottomViewIds(int leftId, int rightId, int topId, int bottomId) {
